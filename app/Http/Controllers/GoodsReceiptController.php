...
class GoodsReceiptController extends Controller
{
    ...
    public function verify($id)
    {
        ...
        return redirect()->route('goods-receipt.index')
            ->with('success', 'Penerimaan Barang berhasil divalidasi & ditandatangani oleh ' . auth()->user()->name)
            ->with('print_url', route('goods-receipt.print', $purchase->id));
    }

    // Show Validate Page
    public function showValidate($id)
    {
        $purchase = Purchase::with(['supplier', 'items.ingredient', 'items.product', 'creator'])->findOrFail($id);

        if ($purchase->status === 'verified') {
            return redirect()->route('goods-receipt.index')->with('info', 'Purchase Order ini sudah divalidasi.');
        }

        return view('goods_receipt.validate', compact('purchase'));
    }

    // Upload Delivery Proof manually
    public function uploadProof(Request $request, $id)
    {
        $request->validate([
            'delivery_proof' => 'required|file|mimes:jpg,jpeg,png|max:2048',
        ]);
        ...
        try {
            if ($purchase->delivery_proof) {
                Storage::disk('public')->delete($purchase->delivery_proof);
            }
            ...
            return back()->with('success', 'Bukti penerimaan berhasil diupload.');
        } catch (\Exception $e) {
