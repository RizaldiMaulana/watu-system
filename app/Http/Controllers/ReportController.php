...
class ReportController extends Controller
{
    ...
    {
        $stocks = Ingredient::orderBy('name')->get();
    ...
    public function jurnal(Request $request)
    {
        ...
        $journals = \App\Models\Journal::with([
            'details.account',
            'details' => function ($q) {
    ...
    private function getBalanceSheetData($endDate)
    {
        ...
        $assets = ChartOfAccount::where('type', 'asset')
            ->withSum([
                'journalDetails' => function ($q) use ($endDate) {
    ...
    private function getPeriodData($type, $startDate, $endDate)
    {
        return ChartOfAccount::where('type', $type)
            ->withSum([
                'journalDetails' => function ($q) use ($startDate, $endDate) {
            ...
            ->map(function ($acc) use ($type) {
                if ($type == 'revenue')
                    $acc->balance = $acc->journal_details_sum_credit - $acc->journal_details_sum_debit;
                else
                    $acc->balance = $acc->journal_details_sum_debit - $acc->journal_details_sum_credit;
                return $acc;
            });
