<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data"
        ...
        <!-- Hidden required fields to pass ProfileUpdateRequest validation -->
        ...
        <div class="space-y-4">
            ...
            <div x-show="mode === 'draw'" style="display: none;">
                <div
                    class="bg-white border-2 border-dashed border-gray-300 rounded-lg relative touch-none hover:border-[#5f674d] transition-colors overflow-hidden">
                    <canvas x-ref="canvas" @mousedown="startDrawing" @mousemove="draw" @mouseup="stopDrawing"
                        ...
                        @touchend.prevent="stopDrawing" class="w-full h-48 cursor-crosshair bg-white"></canvas>

                    <div class="absolute top-2 right-2 flex gap-2">
                        <button type="button" @click="clearCanvas()"
                            class="text-xs bg-white/80 backdrop-blur px-2 py-1 rounded border border-gray-200 text-gray-500 hover:text-red-500 hover:border-red-500 transition-colors shadow-sm">
                            {{ __('Clear') }}
                        </button>
                    </div>

                    <div x-show="isEmpty"
                        class="absolute inset-0 flex items-center justify-center pointer-events-none text-gray-300 text-sm">
                        {{ __('Sign here') }}
                    </div>
                </div>
