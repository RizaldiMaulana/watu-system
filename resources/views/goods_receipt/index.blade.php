<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto" x-data="grTable()">
        ...
        <div class="mb-10">
            <h3 class="font-serif font-bold text-lg text-[#2b2623] mb-4 flex items-center justify-between">
                <span class="flex items-center gap-2">
                    <span
                        class="w-8 h-8 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center text-sm">1</span>
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @foreach($pendingPurchases as $po)
                                    <tr class="hover:bg-[#F9F7F2] transition">
                                        ...
                                        <td class="px-6 py-4 text-right font-mono">Rp
                                            {{ number_format($po->total_amount, 0, ',', '.') }}</td>
                                        <td class="px-6 py-4 text-center">
                                            <a href="{{ route('goods-receipt.create', $po->id) }}"
                                                style="background-color: #F59E0B;"
                                                class="inline-flex items-center gap-2 px-4 py-2 text-white rounded-lg hover:bg-yellow-600 transition font-bold text-xs shadow-md">
                                ...
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        @if($po->status == 'verified')
                                            <span
                                                class="px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-bold">Verified</span>
                                        @else
                                            <span
                                                class="px-3 py-1 bg-amber-100 text-amber-700 rounded-full text-xs font-bold">Menunggu
                                                Validasi</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 text-center">
                                        @if($po->delivery_proof || $po->proof_file)
                                            <div class="flex flex-col items-center gap-1">
                                                <a href="{{ asset('storage/' . ($po->delivery_proof ?? $po->proof_file)) }}"
                                                    target="_blank"
                                                    class="text-[#5f674d] hover:underline font-bold text-xs flex justify-center items-center gap-1">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                ...
                                                @if($po->status !== 'verified')
                                                    <button type="button"
                                                        @click.prevent="openUploadModal({{ $po->id }}, '{{ $po->invoice_number }}', '{{ asset('storage/' . ($po->delivery_proof ?? $po->proof_file)) }}')"
                                        ...
                                        @else
                                            @if($po->status !== 'verified')
                                                <button type="button"
                                                    @click.prevent="openUploadModal({{ $po->id }}, '{{ $po->invoice_number }}', '')"
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        ...
                                        @elseif($po->status === 'verified')
                                            <div class="flex flex-col items-center gap-2">
                                                <div
                                                    class="flex items-center justify-center gap-1 text-green-600 font-bold text-xs">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                            d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                                    </svg>
                                                ...
                                                @if($po->signer)
                                                    <div class="text-[10px] text-gray-500 text-center leading-tight">
                                                        ...
                                                        <span
                                                            class="text-gray-400">{{ $po->signed_at ? \Carbon\Carbon::parse($po->signed_at)->format('d M H:i') : '' }}</span>
                                                    </div>
                                                @endif

                                                <a href="{{ route('goods-receipt.print', $po->id) }}" target="_blank"
                                                    class="px-3 py-1 bg-gray-100 text-gray-600 rounded-md text-[10px] font-bold hover:bg-gray-200 transition flex items-center gap-1">
                                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                            d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2-4h6a2 2 0 012 2v2M7 7h10a2 2 0 012 2v2M7 7H5a2 2 0 00-2 2v2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10">
                                                        </path>
                                                    </svg>
                                                    Print Bukti Terima
                                                </a>
        ...
        {{-- UPLOAD PROOF MODAL --}}
        <div x-show="showUploadModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title"
                ...
                <div x-show="showUploadModal" x-transition:enter="ease-out duration-300"
                    ...
                    <form :action="uploadAction" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                    ...
                                    <div class="mt-2">
                                        <p class="text-sm text-gray-500 mb-4">
                                            Upload foto surat jalan, nota supplier, atau foto barang fisik untuk PO
                                            <span class="font-bold" x-text="poNumber"></span>.
                                        </p>
                                        ...
                                        <div
                                            class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md hover:border-[#5f674d] transition-colors group">
                                            <div class="space-y-1 text-center">
                                                <svg class="mx-auto h-12 w-12 text-gray-400 group-hover:text-[#5f674d]"
                                                ...
                                                <div class="flex text-sm text-gray-600">
                                                    <label for="file-upload"
                                                        class="relative cursor-pointer bg-white rounded-md font-medium text-[#5f674d] hover:text-[#4b523d] focus-within:outline-none">
                                                        <span>Ambil Foto / Pilih Gambar</span>
                                                        <input id="file-upload" name="delivery_proof" type="file"
                        ...
                        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                            <button type="submit"
                                class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-[#5f674d] text-base font-medium text-white hover:bg-[#4b523d] focus:outline-none sm:ml-3 sm:w-auto sm:text-sm">
                                Upload Bukti
                            </button>
        ...
        @if(session('print_url'))
            <script>
                // Open print window automatically after validation
