            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <h3 class="font-bold text-gray-700 mb-4 border-b pb-2 flex justify-between items-center">
                        Bukti Penerimaan
                        @if($purchase->delivery_proof || $purchase->proof_file)
                    ...
                    @if($purchase->delivery_proof || $purchase->proof_file)
                        <div class="relative group cursor-pointer"
                            @click="window.open('{{ asset('storage/' . ($purchase->delivery_proof ?? $purchase->proof_file)) }}', '_blank')">
                            <img src="{{ asset('storage/' . ($purchase->delivery_proof ?? $purchase->proof_file)) }}"
                                class="w-full rounded-lg border border-gray-200 hover:opacity-90 transition">
                            <div
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="flex gap-4 border-b border-gray-200">
                    <button @click="tab = 'details'"
                        :class="tab === 'details' ? 'border-[#5f674d] text-[#5f674d]' : 'border-transparent text-gray-500 hover:text-gray-700'"
                        class="pb-3 px-1 border-b-2 font-bold text-sm transition">
                        Detail Item
                    </button>
                    <button @click="tab = 'document'"
                        :class="tab === 'document' ? 'border-[#5f674d] text-[#5f674d]' : 'border-transparent text-gray-500 hover:text-gray-700'"
                        class="pb-3 px-1 border-b-2 font-bold text-sm transition">
                            ...
                            @foreach($purchase->items as $item)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <span
                                            class="px-2 py-1 {{ $item->ingredient_id ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700' }} rounded text-[10px] font-bold">
                                    ...
                                    <td class="px-6 py-4 text-right font-bold text-gray-700">Rp
                                        {{ number_format($item->quantity * $item->price, 0, ',', '.') }}</td>
                                </tr>
                            @endforeach
                            <tr class="bg-gray-50">
                                <td colspan="4" class="px-6 py-4 text-right font-bold text-gray-600">TOTAL NILAI BARANG
                                </td>
                                <td class="px-6 py-4 text-right font-bold text-xl text-[#5f674d]">Rp
                                    {{ number_format($purchase->items->sum(fn($i) => $i->quantity * $i->price), 0, ',', '.') }}
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div x-show="tab === 'document'" class="bg-gray-100 p-4 rounded-xl border border-gray-200">
                    <div class="bg-white shadow-lg mx-auto max-w-2xl transform scale-95 origin-top-center">
                        ...
                        --}}
                        <iframe src="{{ route('goods-receipt.print', $purchase->id) }}"
                            class="w-full h-[600px] border-0" title="Preview Dokumen"></iframe>
                    </div>
                </div>

                {{-- Action Bar --}}
                <div
                    class="flex items-center justify-between bg-[#F9F7F2] p-6 rounded-xl border border-[#5f674d]/20 mt-8">
                    <div>
                        <h4 class="font-bold text-[#5f674d]">Konfirmasi Validasi</h4>
                        <p class="text-xs text-gray-500 mt-1 max-w-sm">
                            Dengan memvalidasi, stok akan ditambahkan ke sistem dan tanda tangan digital Anda akan
                            dibubuhkan pada dokumen Berita Acara.
                        </p>
                    </div>

                    <div class="flex items-center gap-4">
                        <form action="{{ route('goods-receipt.verify', $purchase->id) }}" method="POST"
                            class="flex items-center gap-4">
                            @csrf

                            @if(!$purchase->delivery_proof && !$purchase->proof_file)
                                <div
                                    class="text-xs text-red-500 font-bold bg-white px-3 py-2 rounded border border-red-200">
                                    ⚠️ Upload Bukti Dulu
                                </div>
                                <button type="button" disabled
                                    class="px-6 py-3 bg-gray-300 text-gray-500 font-bold rounded-xl cursor-not-allowed">
                            ...
                            @else
                                ...
                                <button type="submit" data-confirm="Apakah Anda yakin data sudah benar?"
                                    class="px-6 py-3 bg-[#5f674d] hover:bg-[#4b523d] text-white font-bold rounded-xl shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center gap-2">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
